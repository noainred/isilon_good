...
- **통합/모니터링**: 글로벌 통합 포탈(HQ)·스토리지 어레이 상태(Isilon/PowerStore 등)·Prometheus `/metrics`.
...
> **엔진 선택**: 새 스캔에서 **threads(상세 트리)** 와 **pscan(멀티프로세스·빠른 용량)** 중
> 고를 수 있습니다. pscan 은 GIL 을 우회해 처리량이 크게 오르지만(측정 4워커 5.48배), 루트
> + 1단계 자식까지만 집계합니다(깊은 드릴다운은 threads 엔진). 기본 엔진은 설정 `default_engine`.
...
> 버전별 상세 변경점은 **[CHANGELOG.md](../CHANGELOG.md)** 를 보세요(현재 v1.23.0, 스키마 8).
