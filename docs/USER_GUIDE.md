...
> ⚠ **중지 가능 여부가 다릅니다.** `threads` 는 언제든 **중지·재개**할 수 있지만,
> `pscan` 은 자식 프로세스가 끝까지 돌아 결과를 **마지막에 한 번에** 기록하므로 **시작하면
> 중간에 멈출 수 없습니다**(중지 버튼을 눌러도 그 사실만 안내되고 끝까지 돕니다). 멈출 일이
> 잦으면 `threads`, 빠른 용량 파악이 목적이면 `pscan` 을 쓰세요.

### 4.5 🔬 오토튜닝 (자동으로 최적 방법 선택)
새 스캔을 시작할 때 **오토튜닝**(기본 꺼짐, 체크하면)이 같은 영역에서 **단일 · 멀티프로세스 · 2단 병렬**을
...
- **▶ 이 디렉터리 스캔 시작**: 누르면 **시작 전에 어떤 옵션으로 도는지(경로·엔진·백엔드·
  용량 기준·오토튜닝) 요약 확인창**이 먼저 뜹니다. 확인하면 스캔이 시작되고 관리 개요에 바로
  나타납니다. **pscan(멀티프로세스)이 선택되거나 오토튜닝이 그것을 고를 수 있으면, "시작하면
  중간에 멈출 수 없다"는 경고가 확인창에 함께 표시**됩니다(4.4장).

> 서버를 `--mount-base` 없이 띄우면 어떤 경로든 시작할 수 있고, 지정하면 그 경로
> 밖은 거부됩니다. 폴더 탐색기는 디렉터리 이름만 나열하며 파일 내용은 읽지 않습니다.

> **🔬 오토튜닝(기본 꺼짐 — 필요 시 체크)**: 시작 시 단일/멀티프로세스/2단 병렬을 자동 측정해 가장 빠른
...
- 루트별 최신 스캔 표: 조사 용량, 디스크 사용, 확인%, 상태. 진행 중이면 **중지** 버튼
  (단 `pscan` 스캔은 중지가 안 되며, 눌러도 그 사실을 안내합니다 — 4.4장).
...
- **비교(diff) — 변경 많은 폴더 Top 10**: 같은 루트의 두 스캔을 골라, 어느 폴더의 **크기·파일 수**가
  가장 많이 변했는지 Top 10 으로 봅니다(크기 변화 기준 정렬, 파일 수 증감 함께 표시). 표의 행을
  클릭하거나 경로를 입력하면 **📂 특정 폴더 변화 이력**(그 폴더의 완료 스캔별 크기·파일 수와 증감)을 표로 보여줍니다.
...
| 🔁 자동 업그레이드 감시 폴더 | 이 폴더에 새 버전 `isilon_usage-*.tar.gz/.zip` 을 두면 현재보다 새 버전만 자동 적용 후 재시작. 비우면 끔 |
| 🌐 업데이트(버전) 소스 URL | 새 버전 확인·자동 업그레이드가 `versions.json` 과 패키지를 받는 주소. **비우면 기본**, 폐쇄망이면 **사내 미러 주소**를 넣으세요(‘⬆ 자동 업그레이드’ 카드의 ‘소스 주소’와 동일한 값) |
...
- **🔀 변화 Top**: 직전 완료 스캔 대비 가장 많이 커진/줄어든·신규·삭제 디렉터리(**크기 변화 옆에 파일 수 증감**도 표시).
...
| `POST /api/scan/stop` | `{scan_id}` 스캔 중지(`threads`). `pscan` 은 중간 중지가 안 되어 `{ok:false, engine:"pscan", reason}` 로 사실대로 응답 |
...
| `GET /api/diff?base=&target=` | 두 스캔의 디렉터리별 **크기·파일 수** 증감 비교(변경 많은 폴더 순) |
| `GET /api/folder-history?root=&path=` | 특정 폴더의 완료 스캔별 크기·파일 수 변화 이력(증감 포함) |
...
### 12.3 포탈(HQ) 실행
가장 쉬운 건 **한 줄 설치**(`install_portal.sh` → `/opt/isilon_portal` · 서비스 `isilon-portal` ·
포트 8800; 명령은 [download/README.md](../download/README.md)). 직접 띄우려면:
```bash
python3 -m isilon_usage portal --data-dir /var/lib/isilon_portal --port 8800
# 브라우저: http://<HQ주소>:8800/
```
...
> **자동 설치 + 등록(노드 추가)**: IP·계정만 입력해 엣지를 **자동 설치·기동·등록**할 수도
> 있습니다('② SSH로 자동 설치 + 등록' 또는 생성 스크립트 1회 실행). 이 스크립트는 **설치
> 전에 기존 엣지 프로세스·서비스(현재 `isilon-edge`, 구버전 `isilon_usage`)와 잔여 `serve`
> 프로세스를 먼저 중단**하고 구버전 유닛 파일을 제거합니다 — 포트 충돌·중복 서비스로 인한
> 기동 실패(예: 옛 `isilon_usage.service` 가 포트를 잡고 있어 새 서비스가 안 뜨는 문제)를 막습니다.
...
포탈 접속 보호(로그인 비밀번호)는 **'노드 설정 → 🔒 보안·감사'** 에서 켜고/끕니다. 설정하면
접속이 **읽기 전용**이 되고, 우상단 **로그인** 후에만 변경 작업이 됩니다. 끄려면 **'🔓 비밀번호
해제'**(로그인 필요). 비밀번호를 **잊었다면** 포탈 서버 데이터 폴더의 `portal_settings.json` 에서
`"op_password"` 를 `""` 로 바꾸고 포탈을 재시작하세요. (엣지 대시보드는 설정의 '작업 보호
비밀번호' 체크를 해제·저장하면 풀립니다.)
...
'노드 설정 → 🔁 업그레이드'의 **'지금 업그레이드(+전 엣지)'** 를 누르면 **진행 팝업**이 떠서
① 최신 코드 내려받기 → ② HQ 코드 교체 → ③ 엣지별 전파(엣지마다 ✓/✗) → ④ 재시작 → ✅ 복귀
까지 단계별로 보여줍니다(재시작 공백도 처리, 닫아도 백그라운드 진행).

**인터넷(URL) 자동 업그레이드 + 사설(비공개) 레포.** ‘네트워크 모니터링 → 🌐 인터넷(URL) 자동
업그레이드’ 카드(엣지는 ‘⬆ 자동 업그레이드’ 카드)에서 **소스 URL**(빈값=기본 raw GitHub, 폐쇄망은
사내 미러/사설 레포 폴더 주소)과 확인 주기를 지정하면, 포탈/엣지가 `versions.json` 을 주기적으로 확인해
새 버전을 알리고(‘자동 설치’를 켜면 무인 설치·재시작, 포탈은 전 엣지에 전파) 줍니다. **사설(비공개)
소스**는 **토큰(PAT)** 칸에 토큰을 넣어 저장하세요(저장 여부만 표시·값 비노출, ‘토큰 지우기’로 삭제):
- **비공개 GitHub**: `raw.githubusercontent.com/<owner>/<repo>/<branch>/download` 주소를 그대로 넣으면
  자동으로 contents API 로 인증 조회합니다(브랜치 이름에 `/` 가 있어도 OK — 예: `claude/upbeat-bell-cXX8f`).
- **사내 미러/사설 호스트(Nexus 등)**: `versions.json` 과 `isilon_usage-*.tar.gz` 가 있는 **폴더 주소**를
  넣고, 인증이 필요하면 같은 토큰을 `Authorization: Bearer` 로 보냅니다.

### 12.10 엣지 자기등록 (한 줄 설치 + `--hq`)
엣지 설치 한 줄에 `--hq http://<HQ>:8800` 을 주면, 설치 직후 엣지가 자기 주소·`api_token` 을
포탈에 보내 **스스로 등록**됩니다(포탈→엣지 등록의 반대 방향). 엣지가 자기 토큰을 직접 알려주므로
포탈 폴링이 곧바로 인증돼 **토큰 불일치(401)** 가 생기지 않습니다. 포탈에 로그인 비밀번호가
설정돼 있으면 '보안·감사' 탭의 **enroll 토큰**을 엣지 `--enroll <토큰>` 으로 함께 줍니다.
(정확한 명령은 [download/README.md](../download/README.md).)

### 12.11 오프라인 업그레이드 (포탈 릴리스 폴더)
포탈 서버의 **릴리스 폴더**(기본 `/opt/isilon_release`)에 `isilon_usage-*.tar.gz` 를 두면, 각 엣지가
**인터넷·GitHub·토큰 없이** 포탈에서 받아 업그레이드합니다(포탈→엣지 푸시와 달리 **엣지가
당겨감**). '노드 설정 → 🔁 업그레이드' 탭에서 폴더를 지정·확인할 수 있고(엣지 한 줄 명령도
자동 표시), **엣지 콘솔**에서 그 한 줄(`curl …/api/portal/release | tar …`)을 실행합니다.
