...
275. (법인별 시간당 처리량을 박스 형태로 [이미지: 윈도우 행목록 + '총 사용 용량' 카드]) renderThroughput
     의 펼침 상세를 좁은 행 목록 → '총 사용 용량' 카드(.card+h2+.big+.sub)와 같은 박스 그리드로 변경.
...
281. (깔끔하고 세련된 디자인) dataviz 스킬 참고(카테고리 CVD 검증·상태색 예약·마크 스펙). 정제 레이어 v2를
     portal·dashboard <style> 끝에 추가: 카드 소프트 그림자+헤어라인, 라벨 대문자·자간, KPI tabular-nums,
     미터 녹색글로우 제거·얇은 트랙, 표 여백·헤더 정제, 버튼/배지/내비 정제, 언어토글 헤더 융합. REGION_PAL을
     검증된 카테고리(#3987e5…, CVD·대비 통과)로 교체 + 스택 세그먼트 2px 표면간격(box-shadow inset). 검증기로
     팔레트 확인, 헤드리스 Chromium 렌더 확인. JS·CSS균형·17/17·ruff. 마크업/JS/데이터 무변경. v1.99.21.

282. (오토튜닝 단계를 단일·8p·8p×8t·16p·16p×8t·32p 로 변경) autotune.DEFAULT_LADDER=[(1,1),(8,1),
     (8,8),(16,1),(16,8),(32,1)] 상수 추가, server autotune_start 가 candidates=DEFAULT_LADDER 로 넘김
     (maxp/maxt 제거). _label 이 이미 "8프로세스"·"8프로세스 × 8스레드" 출력. 대시보드 단계표 동적 렌더라
...
287. (지도에 표시하는 기능 추가 — 사용자가 배치해서 저장해서 사용 + 로그 팝업 위치를 좌/우/상/하
     코너로 설정) 포탈 대시보드에 ‘전세계 데이터센터 분포’ 카드: 시안의 도트 세계지도(SVG, 외부
     파일 0)에 노드 마커(상태=색+모양+라벨: ●온라인/◆스캔중/◎오프라인 펄스, id 직접 라벨, 호버
     툴팁, 클릭=노드 열기). ‘배치 편집’=드래그 배치·더블클릭 해제·미배치 트레이 클릭 배치, 저장은
     /api/portal/map-layout(GET 공개/POST 로그인)에 %(0~100) 좌표 전체 교체(클램프·미등록 무시·영속).
     로그 팝업(업그레이드 진행/전 노드 푸시/기록 세부) 위치는 설정 log_popup_pos(center/tl/tr/bl/br)
     로 통일 적용. _load_settings 화이트리스트에 신규 키 반영(재로딩 유실 버그 테스트로 발견·수정).
     단위테스트(_test_map_layout_and_prefs)·실서버 헤드리스 렌더·CSS/JS·ruff 통과. v1.99.27.

288. (설정에서 중앙에서 저장하는 DB 경로 설정하는 메뉴) 설정→모니터링·백업에 ‘중앙 DB 저장 경로’
     카드 — 노드 복제본(replicas/) 저장 위치를 지정(입력+찾아보기 모달 재사용+저장, 적용 경로·복제본
     수 표시, 빈값=기본 복귀). set_replica_dir: 절대경로만·쓰기 검사·재시작 없이 즉시 적용·영속,
     같은 FS+빈 대상이면 rename 즉시 이동, 불가 시 다음 동기화가 자동 재충전(note 안내). 찾아보기
